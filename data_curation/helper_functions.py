...
def patient_series_name_from_filepath(filename):
    try:
        ...
        res = patient_underscore_series_id_from_filepath(filename)
    if res is None:
        return None
    return res[0] + '_' + res[1]


def patient_underscore_series_id_from_filepath(filename):
    basename = os.path.basename(filename)
...
def get_resolution(scan_id, metadata_path=None, df=None, extract_scan_series_id=True, in_plane_res_name='PixelSpacing',
                   subject_id_scan_id=False):
    res_str=None
    try:
        if (df is not None) or (';' not in metadata_path):#only one metadata
            res_str = get_metadata_value(scan_id, column_name=in_plane_res_name, metadata_path=metadata_path, df=df,
                                         extract_scan_series_id=extract_scan_series_id, subject_id_scan_id=subject_id_scan_id)
            spacing = get_metadata_value(scan_id, 'SpacingBetweenSlices', metadata_path, df=df,
                                         extract_scan_series_id=extract_scan_series_id, subject_id_scan_id=subject_id_scan_id)
        else: #multiple data pathes
            met_pathes = metadata_path.split(';')
            for metadata in met_pathes:
                res_str = get_metadata_value(scan_id, column_name=in_plane_res_name, metadata_path=metadata, df=df,
                                             extract_scan_series_id=extract_scan_series_id, subject_id_scan_id=subject_id_scan_id)
                if res_str is not None:
                    spacing = get_metadata_value(scan_id, 'SpacingBetweenSlices', metadata, df=df, extract_scan_series_id=extract_scan_series_id)
                    break
    except:
        print('An exception occured while reading resolution')
...
def get_metadata_value(scan_id, column_name='', metadata_path=None, df=None, extract_scan_series_id=True,
    ...
    if (subject_id is None) or (subject_id_scan_id is True):
        subject_id = scan_id
    print('scan id is: ' + str(subject_id))
    if series_id is not None:
        print('series id is: ' + str(series_id))
    try:
        if(df is None):
            df = pd.read_csv(metadata_path, encoding ="unicode_escape")
    except:
    ...
    if (series_id is not None) and ('Series' in df.columns):
        if subject_id.isnumeric() is True:
            row_df = df[(df['Subject'] == int(subject_id)) & (df['Series'] == int(series_id))]
            if row_df.empty is True:
                row_df = df[(df['Subject'] == subject_id) & (df['Series'] == int(series_id))]
        else:
            row_df = df[(df['Subject'] == subject_id) & (df['Series'] == int(series_id))]
    ...
    try:
        ...
        print("no information about " + column_name + " in scan " + str(subject_id))
        return None
