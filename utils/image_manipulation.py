...
def connected_components_above_thresh(data, min_num_voxels):
    """
    Maintain only connected components with number of voxels above threshold
    Supports both 2D and 3D data
    :param data: input mask
    :param min_num_voxels: minimum number of voxels
    :return: results with connected components above threshold
    """
    ...
    if len(data.shape) == 3:
        ...
        raise Exception('input length is not 3D or 2D')

    labeled_array, num_features = label(data, structure=s)
    (unique, counts) = np.unique(labeled_array, return_counts=True)
    components_num = []
    for i in range(0, len(unique)):
        if unique[i] > 0 and counts[i] > min_num_voxels:
            seg[labeled_array == i] = i
            components_num.append(i)

    return seg, components_num
