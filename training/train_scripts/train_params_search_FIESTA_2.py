...
def train_data(train_generator, validation_generator, n_train_steps, n_validation_steps, overwrite,
            ...
            patience, early_stop, training_file, validation_file, test_file, data_dir, old_model_path, save_best_only, labmda_ac, beta_ac, gamma_ac, theta_ac):
    ...
    if not overwrite and len(glob.glob(model_file + '*.hdf5')) > 0:
        ...
        model = load_old_model(model_path, loss, initial_learning_rate, dropout_rate, input_shape, chosen_model, lambda_ac=labmda_ac, beta_ac=beta_ac, gamma_ac=gamma_ac, theta_ac=theta_ac)
    else:
...
if __name__ == '__main__':
    ...
    log_dir = '../../../log/params_search_FIESTA_2'
