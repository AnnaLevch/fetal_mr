...
def config():
    ...
    labmda_ac = 1 #default parameter for AC loss
    beta_ac = 1
    gamma_ac = 1
    theta_ac=0
    loss = {
...
def train_data(train_generator, validation_generator, n_train_steps, n_validation_steps, overwrite,
            ...
            patience, early_stop, training_file, validation_file, test_file, data_dir, old_model_path,
               save_best_only, labmda_ac, beta_ac, gamma_ac, theta_ac, alpha_q, u_th, denoising, depth, drop_xy_levels,
               mask_shape, t, concat_mask, band, isotropic, mask_output, reduce_plateau_with_restarts, dynamic_loss,
               th_binarization):
 ...
 #   os.environ["CUDA_VISIBLE_DEVICES"]="0"
    ...
    if dynamic_loss is True:
        gamma_dyn = DynamicParam(gamma_ac)
    else:
        gamma_dyn = None
    ...
    if not overwrite and len(glob.glob(model_file + '*.hdf5')) > 0:
        ...
        model = load_old_model(model_path, loss, initial_learning_rate, dropout_rate, input_shape, chosen_model,
                               lambda_ac=labmda_ac, beta_ac=beta_ac, gamma_ac=gamma_ac, theta_ac=theta_ac, u_th=u_th,
                               denoising=denoising, band=band, drop_xy_levels=drop_xy_levels, alpha_q=alpha_q,
                               mask_shape=mask_shape, gamma_dyn=gamma_dyn, th_binarization=th_binarization)
    else:
        ...
        model = model_func(input_shape,
                           ...
                           u_th=u_th,
                           gamma_dyn=gamma_dyn,
                           th_binarization=th_binarization,
...
def generate_data(scans_dir, data_dir, normalization, training_modalities, ext, overwrite, batch_size,data_split_test, data_split_validation, validation_file, training_file, test_file,
                  ...
                  patches_per_epoch, is_3D, truth_index, truth_size, truth_crop, categorical, preprocess, scale, rescale_res, metadata_path, isotropic, samples_weights):
    ...
    data_file_opened = create_load_hdf5(normalization=normalization, data_dir=data_dir, scans_dir=scans_dir, train_modalities=training_modalities, ext=ext,
                                        overwrite=overwrite, preprocess=preprocess, scale=scale, rescale_res=rescale_res, metadata_path=metadata_path, isotropic=isotropic)
...
if __name__ == '__main__':
    ...
    log_dir = '/media/df4-dafna/Anna_for_Linux/Bella/training/log/'
