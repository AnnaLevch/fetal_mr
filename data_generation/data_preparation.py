        ...
        for modality in train_modalities:
            ...
            for single_modality in modalities:
                ...
                if os.path.exists(file_path):
                    ...
                    subject_files.append(file_path + '.gz')
                    break
...
def write_image_data_to_file(image_files, data_storage, subject_ids, scale=None, preproc=None, rescale_res=None, metadata_path=None, default_res= [1.56,1.56,3], isotropic=False):
    for subject_id, set_of_files in zip(subject_ids, image_files):
        ...
        if rescale_res is not None and metadata_path is not None:
            ...
            print('extracted resolution is: ' + str(curr_resolution))
...
def write_data_to_file(training_data_files, out_file, subject_ids=None, normalize='all', scale=None, preproc=None, rescale_res=None, metadata_path=None, isotropic=False):
    """
    ...
    write_image_data_to_file(training_data_files, data_dict, subject_ids, scale=scale, preproc=preproc, rescale_res=rescale_res, metadata_path=metadata_path, isotropic=isotropic)
...
def create_load_hdf5(normalization, data_dir, scans_dir, train_modalities, ext, overwrite=False, preprocess=None,
                     scale=None, rescale_res=None, metadata_path=None, isotropic=False):
    """
    ...
    if overwrite or not os.path.exists(data_file):
        ...
        _, (mean, std) = write_data_to_file(training_files, data_file, subject_ids=subject_ids,normalize=normalization,
                                            preproc=preproc_func, scale=scale, rescale_res=rescale_res, metadata_path=metadata_path,
                                            isotropic=isotropic)
