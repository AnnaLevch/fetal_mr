...
def pad_samples(data_dict, patch_shape, truth_downsample, samples_pad):
    ...
    for key in data_dict:
        ...
        if('mask' in data_dict[key]):
            data_dict[key]['mask'] = \
                np.pad(data_dict[key]['mask'], [(_, _) for _ in padding], 'constant', constant_values=0)
    ...
    for key in data_dict:
        ...
        if('mask' in data_dict[key]):
            ...
            data_dict[key]['mask'] = \
                np.pad(mask, [(_, _) for _ in np.ceil(np.maximum(np.subtract(patch_shape, mask.shape) + 1, 0) / 2).astype(int)],
                    'constant', constant_values=0)
...
def add_data(x_list, y_list, mask_list, data_dict, index, truth_index, truth_size=1, augment=None, patch_shape=None,
             skip_blank=True, truth_downsample=None, truth_crop=True, prev_truth_index=None, prev_truth_size=None,
    ...
    :return:
    """
    data, truth, mask = get_data_from_file(data_dict, index, patch_shape=patch_shape)

    patch_corner = [
        np.random.randint(low=low, high=high)
        for low, high in zip((0, 0, 0), truth.shape - np.array(patch_shape))
    ]
    if augment is not None:
        ...
        data, truth, prev_truth, mask = \
            augment_data(data, truth,
                         ...
                         min_crop_pad=augment.get('min_crop_pad', None),
                         max_crop_pad=augment.get("max_crop_pad", None),
...
def weighted_random_list_generator(index_list, weights):
    while True:
        np.random.seed()
        yield from random.choices(index_list, weights, k=len(index_list))
...
def data_generator(data_dict, index_list, batch_size=1, n_labels=1, labels=None, augment=None, patch_shape=None,
                   ...
                   drop_easy_patches=False, is3d=False, samples_weights_path=None):
    if samples_weights_path is not None:
        samples_weights = numbers_list_load(samples_weights_path)
        index_generator = weighted_random_list_generator(index_list, samples_weights)
    elif shuffle_index_list:
        index_generator = random_list_generator(index_list)
    else:
        list_generator(index_list)
...
def get_training_and_validation_generators(data_dict: dict, batch_size, n_labels, training_keys_file, validation_keys_file,
                                           ...
                                           is3d=False, samples_weights=None,
    ...
    training_generator = \
        data_generator(data_dict=data_dict, index_list=training_list, batch_size=batch_size, augment=augment,
                       ...
                       drop_easy_patches=drop_easy_patches_train, samples_weights_path=samples_weights)
    validation_generator = \
        data_generator(data_dict=data_dict, index_list=validation_list, batch_size=validation_batch_size,
                       ...
                       truth_downsample=truth_downsample, truth_crop=truth_crop,
                       is3d=is3d,categorical=categorical,
                       drop_easy_patches=drop_easy_patches_val, samples_weights_path=None)
