...
def get_distortion_affine(data, affine, flip_axis=None, scale_factor=None, rotate_factor=None, translate_factor=None):
...
def cut_bounding_box(img, start, end):
    ...
    return crop_img_to(img_nii, slices, copy=True)


def crop_start_end(min_crop, max_crop):
    x_rand = np.random.uniform(min_crop[0],max_crop[0], 2)
    y_rand = np.random.uniform(min_crop[1],max_crop[1], 2)
    z_rand = np.random.uniform(min_crop[2],max_crop[2], 2)
    padding_start = np.array([int(x_rand[0]), int(y_rand[0]), int(z_rand[0])])
    padding_end = np.array([int(x_rand[1]), int(y_rand[1]), int(z_rand[1])])

    return padding_start, padding_end


def crop_data(data,mask, padding_start, padding_end):
    """
    This funciton crops image while preserving ground truth
    The size of the data around ground truth is determined by padding_start and padding_end
    :param data:
    :param mask:
    :param padding_start:
    :param padding_end:
    ...
    bbox_start = np.maximum(bbox_start - padding_start, 0)
    bbox_end = np.minimum(bbox_end + padding_end, mask.shape)

    volume = cut_bounding_box(data, bbox_start, bbox_end)
    truth = cut_bounding_box(mask, bbox_start, bbox_end)

    return volume.get_data(), truth.get_data()
...
def augment_data(data, truth, data_min, data_max, mask=None, scale_deviation=None, iso_scale_deviation=None,
                 ...
                 piecewise_affine=None, elastic_transform=None, intensity_multiplication_range=None, min_crop_pad=None,
                 max_crop_pad=None,gaussian_filter=None, coarse_dropout=None, data_range=None, truth_range=None, prev_truth_range=None):
    ...
    ##################################
    #should I pad the images afterwards?
    ################################
    #Cropping augmentation
    if min_crop_pad is not None and max_crop_pad is not None:
        padding_start, padding_end = crop_start_end(min_crop_pad, max_crop_pad)
        data, truth = crop_data(data,truth, padding_start, padding_end)

    image, affine = data, np.eye(4)
    distorted_data, distorted_affine = get_distortion_affine(image, affine,
                                                             flip_axis=flip_axis,
    ...
    distorted_truth_data, distorted_truth_affine = get_distortion_affine(truth_image, truth_affine,
                                                                         flip_axis=flip_axis,
                                                                         scale_factor=scale_factor,
                                                                         rotate_factor=rotate_factor,
                                                                         translate_factor=translate_factor)
    ...
    if mask is None:
        ...
        distorted_mask_data, distorted_mask_affine = get_distortion_affine(mask_image, mask_affine,
                                                                           flip_axis=flip_axis,
                                                                           scale_factor=scale_factor,
                                                                           rotate_factor=rotate_factor,
                                                                           translate_factor=translate_factor)
        if truth_range is None:
            ...
            mask_data = interpolate_affine_range(distorted_mask_data, distorted_mask_affine,
                                                 truth_range, order=0, mode='constant', cval=0)#TODO: Change cval to max value instead of hardcoded 100
