...
from data_generation.augment import contrast_augment
from data_generation.preprocess import window_1_99, normalize_data
from evaluation.eval_utils.eval_functions import dice_coefficient, calc_dice_per_slice
from evaluation.eval_utils.postprocess import *
from training.train_functions.training import load_old_model, get_last_model_path
from utils.read_write_data import pickle_load, load_nifti
...
def batch_iterator(indices, batch_size, data_0, patch_shape, truth_0, prev_truth_index, truth_patch_shape, input_mask):
    i = 0
    while i < len(indices):
        batch = []
        curr_indices = []
        while len(batch) < batch_size and i < len(indices):
            ...
            if input_mask is not None:
                mask_patch = get_patch_from_3d_data(input_mask, patch_shape=patch_shape, patch_index=curr_index)
...
def patch_wise_prediction(model: Model, data, patch_shape, overlap_factor=0, batch_size=5,
                          truth_data=None, prev_truth_index=None, prev_truth_size=None, input_mask=None):
    """
    :param truth_data:
    :param permute:
    ...
    b_iter = batch_iterator(indices, batch_size, data_0, patch_shape,
                            truth_0, prev_truth_index, truth_patch_shape, input_mask)
    ...
    with tqdm(total=len(indices)) as pbar:
        for [curr_batch, batch_indices] in tb_iter:
            ...
            if is3d:
                curr_batch = np.asarray(curr_batch)
                if input_mask is not None:#if we have 2 inputs - volume and mask
                    mask_input = curr_batch[:,1,:,:,:]
                    data_input = curr_batch[:,0,:,:,:]
                    data_input = np.expand_dims(data_input, 1)
                    mask_input = np.expand_dims(mask_input, 1)
                    curr_batch = [data_input, mask_input]
                else:
...
def predict_augment(data, model, overlap_factor, patch_shape, num_augments=32, input_mask=None, visualize=False):
    data_max = data.max()
    data_min = data.min()
    data = data.squeeze()
    ...
    for _ in range(num_augments):
        ...
        # spatial augmentations. Apply also to mask if exists.
        ...
        if input_mask is not None:
            aug_mask = flip_it(input_mask, to_flip)
        if to_transpose:
            curr_data = curr_data.transpose([1, 0, 2])
            if input_mask is not None:
                aug_mask = aug_mask.transpose([1, 0, 2])
        curr_data = ndimage.rotate(curr_data, rotate_factor, order=order, reshape=False)
        if input_mask is not None:
            aug_mask = ndimage.rotate(aug_mask, rotate_factor, order=order, reshape=False)

        curr_prediction = patch_wise_prediction(model=model, data=curr_data[np.newaxis, ...], input_mask=input_mask,
                                                overlap_factor=overlap_factor, patch_shape=patch_shape).squeeze()

        if visualize is True:
            transformed_data += [curr_data, curr_prediction.squeeze()]
